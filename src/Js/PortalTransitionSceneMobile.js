...
export class PortalTransitionSceneMobile extends PortalTransitionSceneBase {
    ...
    if (section.type === "team") {
        ...
        const updateQuotePositions = () => {
          ...
          fragments.forEach((fragment) => {
            ...
            const progress = Math.max(
              0,
              Math.min(1, (startZ - this.camera.position.z) / (startZ - endZ))
            );
      ...
      if (section.secondaryImages) {
        ...
        const updatePositions = () => {
          ...
          mainMesh.position.x =
            section.position === "left" ? progress * 4 : -(progress * 4);
  ...
  createQuoteMeshes(quotes) {
    return quotes.map((quote) => {
      ...
      tempCtx.font = "italic 40px Georgia";
      ...
      // Create canvases with dynamic height
      const backgroundCanvas = document.createElement("canvas");
      const textCanvas = document.createElement("canvas");
      const canvasWidth = 1024;
      const canvasHeight = lines.length * lineHeight + padding * 3.5;

      backgroundCanvas.width = textCanvas.width = canvasWidth;
      backgroundCanvas.height = textCanvas.height = canvasHeight;
      ...
      txtCtx.font = "italic 40px Georgia";
      ...
      lines.forEach((line, i) => {
        ...
        txtCtx.fillText(line, textCanvas.width / 2, y);
      });
      ...
      // Calculate aspect ratio and adjust geometry
